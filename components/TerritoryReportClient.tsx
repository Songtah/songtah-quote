  ...
  const exportReport = () => {
    fetch('/api/audit-pageview', {
      ...
      body: JSON.stringify({
        ...
        title: `匯出轄區報表：${territory.city}${territory.district}｜${reportLabel}｜${exportFormat.toUpperCase()}`,
      }),
    }).catch(() => {})
    if (exportFormat === 'csv') {
      downloadCustomerCsv(
      ...
      <section className="report-controls mx-auto mb-5 max-w-[210mm] rounded-3xl bg-white p-4 shadow-lg ring-1 ring-stone-900/[0.06] sm:p-5">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex items-start gap-3">
            ...
            <div><p className="text-[11px] font-bold uppercase tracking-widest text-stone-400">匯出設定</p><h1 className="mt-1 text-lg font-bold">選好內容與檔案格式</h1><p className="mt-1 text-sm text-stone-500">PDF 保留統計版面；CSV 適合篩選與後續整理。</p></div>
          </div>
        ...
        {(mode === 'both' || mode === 'summary') && (
          <section className="avoid-break mt-6">
            ...
            <div className="mt-3 grid grid-cols-3 gap-3">
              {typeCards.map((item) => <div key={item} className="rounded-2xl bg-stone-50 p-4"><p className="text-xs font-semibold text-stone-500">{item}</p><p className="mt-2 text-2xl font-bold text-stone-800">{marketByType[item].toLocaleString()}</p><p className="mt-1 text-[10px] text-stone-400">主檔可匯出 {customerCounts[item].toLocaleString()} 家</p></div>)}
            </div>

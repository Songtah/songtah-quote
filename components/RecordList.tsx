'use client'
import { motion, AnimatePresence } from 'framer-motion'
import { staggerFast, listItem, fadeUp } from '@/lib/motion'

export function RecordList({
  title,
  items,
  emptyLabel,
}: {
  title: string
  items: Array<{ id: string; title: string; meta: string }>
  emptyLabel: string
}) {
  return (
    <motion.section variants={fadeUp} initial="hidden" animate="show" className="panel p-6">
      <div className="mb-4 flex items-center justify-between gap-3">
        <div>
          <p className="eyebrow mb-1">Overview</p>
          <h3 className="text-base font-semibold text-stone-900">{title}</h3>
        </div>
        <div className="rounded-full bg-stone-100 px-3 py-1 text-xs font-medium text-stone-500">
          {items.length} 筆
        </div>
      </div>
      {items.length === 0 ? (
        <div className="mt-4 rounded-lg border border-dashed border-stone-200 bg-stone-50 px-4 py-8 text-center text-sm text-stone-400">
          {emptyLabel}
        </div>
      ) : (
        <motion.ul
          variants={staggerFast}
          initial="hidden"
          animate="show"
          className="mt-2 divide-y divide-stone-100 list-none"
        >
          {items.map((item) => (
            <motion.li key={item.id} variants={listItem} className="py-3">
              <div className="text-sm font-medium text-stone-900">{item.title}</div>
              <div className="mt-0.5 text-sm text-stone-500">
                {item.meta || '尚未補充資訊'}
              </div>
            </motion.li>
          ))}
        </motion.ul>
      )}
    </motion.section>
  )
}

...
export default function MyTerritoriesPanel() {
  ...
  const [listDialog, setListDialog] = useState<{ scope: 'territories' | 'customers'; area?: { city: string; district: string } } | null>(null)
  ...
  const openList = async (nextScope: 'territories' | 'customers', area?: { city: string; district: string }) => {
    listRequestRef.current?.abort()
    ...
    setListLoading(true)
    try {
      const response = await fetch(`/api/bd/my-customer-list?scope=${nextScope}`, { signal: controller.signal })
      const json = await response.json()
        ...
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-stone-950/30 p-0 backdrop-blur-sm sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="customer-list-title" onMouseDown={(event) => { if (event.target === event.currentTarget) closeList() }}>
          <section className="flex max-h-[92dvh] w-full max-w-3xl flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl ring-1 ring-stone-900/[0.08] sm:rounded-3xl">
            <header className="flex items-start gap-3 border-b border-stone-900/[0.06] px-5 py-4 sm:px-6">
              <div className="min-w-0 flex-1"><p className="text-[11px] font-bold uppercase tracking-widest text-stone-400">客戶清單</p><h2 id="customer-list-title" className="mt-1 text-xl font-bold text-stone-800">{listDialog.area ? `${listDialog.area.city}${listDialog.area.district}` : listDialog.scope === 'customers' ? '我的既有客戶' : '我的全部轄區'}</h2><p className="mt-1 text-sm text-stone-400">只顯示你有權查看的客戶，共 {visibleListItems.length.toLocaleString()} 家。</p></div>

...
type Ownership = 'mine' | 'claimable' | 'others'
type CustomerListItem = {
  id: string; name: string; city: string; district: string; type: string
  status: string; devStage: string; salesperson: string
  ownership: Ownership
  claimable: boolean
  /** 負責業務為「公司」「盤商」等特殊池，不是同事，也不可自助認領 */
  pooled: boolean
}
type ListSummary = { total: number; mine: number; claimable: number; others: number }
const OWNERSHIP_TABS: { value: '' | Ownership; label: string }[] = [
  { value: '', label: '全部' },
  { value: 'mine', label: '我的客戶' },
  { value: 'claimable', label: '尚未認領' },
  { value: 'others', label: '其他業務' },
]
...
export default function MyTerritoriesPanel() {
  ...
  const [listOwnership, setListOwnership] = useState<'' | Ownership>('')
  const [listSummary, setListSummary] = useState<ListSummary | null>(null)
  const [coverage, setCoverage] = useState<Map<string, { claimed: number; unclaimed: number }>>(new Map())
  ...
  // 認領覆蓋率另外抓：要掃客戶庫，比轄區本身慢很多。抓失敗就不顯示這一列，
  // 不擋原有的市場數字與清單功能。
  useEffect(() => {
    let cancelled = false
    fetch('/api/bd/territory-coverage')
      .then((response) => (response.ok ? response.json() : null))
      .then((json) => {
        if (cancelled || !json?.items) return
        setCoverage(new Map(
          json.items.map((item: { city: string; district: string; claimed: number; unclaimed: number }) =>
            [`${item.city}|${item.district}`, { claimed: item.claimed, unclaimed: item.unclaimed }]),
        ))
      })
      .catch(() => {})
  ...
  const openList = async (nextScope: 'territories' | 'customers' | 'claimable', area?: ListArea) => {
    ...
    setListType(type)
    setListSummary(null)
    // 從「認領此區客戶」進來時直接停在尚未認領分頁
    setListOwnership(nextScope === 'claimable' ? 'claimable' : '')
    setDetailId('')
    setDetail(null)
    setDetailError('')
    setChecked(new Set())
  ...
  const visibleListItems = useMemo(() => listItems.filter((item) =>
    ...
    (!listOwnership || item.ownership === listOwnership) &&
    (!listSearch.trim() || `${item.name}${item.city}${item.district}${item.type}`.toLowerCase().includes(listSearch.trim().toLowerCase()))
  ), [listDialog, listItems, listSearch, listType, listOwnership])

  // 認領只能勾未認領者：切換分頁或清單重載時，清掉已不在可見範圍的勾選
  useEffect(() => {
    setChecked((prev) => {
      if (prev.size === 0) return prev
      const allowed = new Set(listItems.filter((item) => item.claimable).map((item) => item.id))
        ...
        {visible.length === 0 ? (
          ...
          <>
            ...
            <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
              {visible.map((territory) => <article key={territory.id || `${territory.city}|${territory.district}`} className="rounded-2xl bg-white p-4 shadow-sm ring-1 ring-stone-900/[0.05]"><div className="flex items-center justify-between gap-3"><div><h3 className="font-bold text-stone-800">{territory.city}{territory.district}</h3>{scope === 'team' && <p className="mt-0.5 text-xs text-stone-400">{territory.salesperson}</p>}</div><div className="text-right"><p className="text-xl font-bold text-brand-700">{marketCount(territory).toLocaleString()}</p><p className="text-[11px] text-stone-400">{type || (areaSource === 'customers' ? '我的客戶' : '全部市場')}</p></div></div>{(() => {
                const cov = coverage.get(`${territory.city}|${territory.district}`)
                if (!cov) return null
                return (
                  <div className="mt-2.5 flex items-center gap-1.5 text-[11px]">
                    <span className="rounded-full bg-brand-50 px-2 py-0.5 font-semibold text-brand-700">已認領 {cov.claimed.toLocaleString()}</span>
                    <span className={`rounded-full px-2 py-0.5 font-semibold ${cov.unclaimed > 0 ? 'bg-stone-900 text-white' : 'bg-stone-100 text-stone-400'}`}>未認領 {cov.unclaimed.toLocaleString()}</span>
                  </div>
                )
              })()}<button onClick={() => openList(areaSource === 'customers' ? 'customers' : 'territories', { id: territory.id, city: territory.city, district: territory.district, salesperson: territory.salesperson })} className="mt-3 flex min-h-10 w-full items-center justify-center gap-2 rounded-full bg-stone-50 px-3 py-2 text-xs font-semibold text-stone-600 transition-all hover:bg-brand-50 hover:text-brand-700 active:scale-95"><ListFilter className="size-3.5" />{scope === 'team' ? `查看 ${territory.salesperson} 客戶` : '查看我的客戶'}</button>{scope === 'mine' && canClaim && <button onClick={() => openList('claimable', { id: territory.id, city: territory.city, district: territory.district, salesperson: territory.salesperson })} className="mt-2 flex min-h-10 w-full items-center justify-center gap-2 rounded-full bg-brand-50 px-3 py-2 text-xs font-semibold text-brand-700 transition-all hover:bg-brand-100 active:scale-95"><UserPlus className="size-3.5" />認領此區客戶</button>}</article>)}
            </div>
          </>
        )}
      </div>
      ...
      {listDialog && (
        <div className="fixed inset-0 z-[100] flex items-end justify-center bg-stone-950/30 p-0 backdrop-blur-sm sm:items-center sm:p-5" role="dialog" aria-modal="true" aria-labelledby="customer-list-title" onMouseDown={(event) => { if (event.target === event.currentTarget) closeList() }}>
          <section className="flex max-h-[92dvh] w-full max-w-3xl flex-col overflow-hidden rounded-t-3xl bg-white shadow-2xl ring-1 ring-stone-900/[0.08] sm:rounded-3xl">
            ...
            <div className="border-b border-stone-900/[0.06] px-5 py-3 sm:px-6">
              <input className="input-soft block w-full" value={listSearch} onChange={(event) => setListSearch(event.target.value)} placeholder="搜尋客戶名稱、地區或類型" />
              {/* 認領彈窗本來就只列未認領，不需再分歸屬 */}
              {listDialog.scope !== 'claimable' && listSummary && (
                <div className="mt-3 flex gap-1 overflow-x-auto">
                  {OWNERSHIP_TABS.map((option) => {
                    const count = option.value === '' ? listSummary.total : listSummary[option.value]
                    return (
                      <button
                        key={option.value || 'all'}
                        onClick={() => setListOwnership(option.value)}
                        className={`min-w-max rounded-full px-3 py-1.5 text-xs font-semibold transition-all active:scale-95 ${listOwnership === option.value ? 'bg-stone-900 text-white' : 'bg-stone-50 text-stone-500 hover:bg-brand-50 hover:text-brand-700'}`}
                      >
                        {option.label}
                        <span className={`ml-1.5 tabular-nums ${listOwnership === option.value ? 'text-white/70' : 'text-stone-400'}`}>{count.toLocaleString()}</span>
                      </button>
                    )
                  })}
                </div>
              )}
              <div className="mt-2 flex gap-1 overflow-x-auto">{TYPES.map((option) => <button key={option.value || 'all'} onClick={() => setListType(option.value)} className={`min-w-max rounded-full px-3 py-1.5 text-xs font-semibold transition-all active:scale-95 ${listType === option.value ? 'bg-brand-500 text-white' : 'bg-stone-50 text-stone-500 hover:bg-brand-50 hover:text-brand-700'}`}>{option.label}</button>)}</div>
            </div>

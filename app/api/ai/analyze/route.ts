...
import { listVisits, type Visit } from '@/lib/system-notion'
...
export type OverviewAnalysis = {
  mode: 'overview'
  analyzedCount: number   // 實際分析筆數（由 API 填入）
  period: string
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (mode === 'overview') {
      // 直接從 Notion 抓全部符合條件的紀錄（不受 UI 分頁限制）
    ...
    // overview：精簡格式最多 500 筆；customer：詳細格式最多 60 筆
...
只回傳 JSON，不要加其他文字。`
    ...
    try {
      ...
      result = mode === 'customer'
        ...
        : { mode: 'overview', analyzedCount: visitsSlice.length, ...parsed }
    } catch {

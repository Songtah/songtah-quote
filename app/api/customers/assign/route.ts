 ...
 * body: { city, district, salesperson, type?, status?, excludeClosed?, dryRun? }
...
export const POST = withApiAuth({ roles: ['中央管理', '總經理'] }, async (req: NextRequest, _ctx, session) => {
  ...
  try {
    ...
    await logAuditEvent({
      ...
      after: { city, district, salesperson, assigned, skipped: skipped.length, type: b.type, status: b.status },
    }).catch(() => {})

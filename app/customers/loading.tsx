    ...
    <div className="min-h-screen bg-stone-50">
      ...
      <div className="sticky top-0 z-20 border-b border-stone-200 bg-white">
        <div className="mx-auto flex max-w-7xl items-center gap-3 px-4 sm:px-6 py-2.5 sm:py-3">
          ...
          <div className="h-5 w-24 bg-stone-100 rounded animate-pulse hidden sm:block" />
        </div>
        <div className="mx-auto max-w-7xl px-3 sm:px-6 pb-2">
          <div className="h-8 w-64 bg-stone-100 rounded-full animate-pulse" />
        </div>
      </div>
      ...
      <main className="mx-auto max-w-7xl px-4 sm:px-6 py-4 sm:py-8">
        ...
        <div className="space-y-4">
          {/* 搜尋框 */}
          <div className="h-12 bg-white rounded-xl border border-stone-200 animate-pulse" />

          {/* Type pills */}
          <div className="flex gap-2">
            {[4, 3, 4, 3].map((w, i) => (
              <div key={i} className={`h-8 w-${w * 4} bg-stone-100 rounded-full animate-pulse`} />
            ))}
          </div>
          ...
          {/* 客戶列表 */}
          <div className="bg-white rounded-2xl border border-stone-200 overflow-hidden divide-y divide-gray-50">
            {Array.from({ length: 10 }).map((_, i) => (
              <div key={i} className="flex items-center gap-4 px-5 py-3.5">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  />
                  <div
                    className="h-3 bg-stone-50 rounded animate-pulse"
                    style={{ width: `${20 + (i * 11) % 25}%` }}
                  />
                </div>
                <div className="w-4 h-4 bg-stone-100 rounded animate-pulse shrink-0" />
              </div>
            ))}
          </div>
        </div>
